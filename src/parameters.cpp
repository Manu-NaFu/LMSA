    ...
    ui(new Ui::parameters)
{
   ...
   /* ui->x_lev->setVisible(false);
    ...
    ui->text_heruistic->setText("");
    ui->text_safe_dist->setText("");
    ui->text_perc_oc->setText("");
...
void parameters::on_b_accept_clicked()
{
    ...
    if (ui->text_heruistic->text().length() > 0)
    {
        if (ui->text_heruistic->text() == 'e')
            heuristic_ = 'e';
        else if (ui->text_heruistic->text() == 'm')
            heuristic_ = 'm';
        else
            error = true;
    }

    if (ui->text_safe_dist->text().length() > 0)
    {
        if (ui->text_safe_dist->text().toInt() > 0)
            safety_distance_ = ui->text_safe_dist->text().toInt();
        else
            error = true;
    }

    if (ui->text_perc_oc->text().length() > 0)
    {
        if (ui->text_perc_oc->text().toInt() > 0)
            perc_occupancy_ = ui->text_perc_oc->text().toInt();
        else
            error = true;
    }
