...
namespace slam
{
    ...
    class Position
    {
        ...
        //Observers
            ...
            inline const cv::Point getPos() const {return cv::Point(x_,y_);} //Returns the component Y of the system.
    ...
    class Map
    {
        ...
        //Constructors
            //Default constructor given width and height components.
            //Initiates every pixel to 0;
            Map(const int &width = 800, const int &height = 800)
            {
                cv::Mat map(width, height, CV_8UC3);
                this->setMap(map);
                cv::Mat oc(width, height, CV_8UC1);
                this->oc_ = oc;
        ...
        //Modifiers
            ...
            inline const Position getPosOut(const int x0, const int y0, const int x1, const int y1) const
            {
                ...
                {
                    if (it.pos().x > this->getCols() || it.pos().y > this->getRows() || it.pos().x < 1 || it.pos().y < 1)
                        break;
                }
                return Position(it.pos().x, it.pos().y);
            }
            ...
            void update(slam::Position P)
            {
                for(int i=0;i<this->getRows();i++)
                    for(int j=0;j<this->getCols();j++)
                    {
                        uchar oc_value = this->getPixelOc(i,j);
                        if (oc_value != 0)
                        {
